...
multipolar_runtime/
  ...
  scenarios.py                 # Scenario Zoo loader / runner
...
Scenario Zoo preset loader
client-side injection / isolation / bounded commitment what-if controls
Story Arc beats for scenario-shaped demos
Branch Lab save / rollback / URL export / JSON snapshot export
Protocol Tuning sliders for domination, translation haze, and stalemate caps
Invariant status
...
Scenario Zoo entries package a cast of agents, a query sequence, runtime
thresholds, story beats, and public mock projections into a runnable experiment.

```bash
python run_multipolar_runtime.py list-scenarios

python run_multipolar_runtime.py run \
  --scenario civic_deliberation \
  --output runtime_out_civic

python run_multipolar_runtime.py run-scenario inner_council \
  --output runtime_out_inner
```
...
The viewer includes preset paths for the generated example outputs under:

```text
examples/scenario_zoo/
```

In the viewer, Scenario Zoo outputs also unlock:

```text
Story Arc        scenario premise, stakes, and round-by-round beats
Branch Lab       save a local what-if branch, roll back to it later, or export it
Protocol Tuning  tighten or relax local domination / translation / stalemate thresholds
```

Branch URL export writes a compact recipe into the viewer URL so another reader
can reopen the same base output plus the local branch additions. Branch JSON
export writes a full snapshot for audit, archiving, or offline inspection.
